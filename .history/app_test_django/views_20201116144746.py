...
def view_dashboard(request, trip_id):
    ...
    user = User.objects.get(id=request.session['trip_id'])
    trip_from_db = Trip.objects.all()

    context = {
        "user": user,
        "trip":trip_from_db
    }
    print(trip_from_db.id)
    return render(request, "dash_board.html", trip_id, context)
