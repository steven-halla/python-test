...
def view_edit_trip(request, trip_id):
    ...
    user = User.objects.get(id=request.session['user_id'])
    trip = Trip.objects.filter(uploaded_by_id=request.session['user_id'])
    context = {
        "user": user,
        "user_trips": user_trips
    }
...
def edit_trip(request, trip_id):
    ...
    trip_to_update.enddate = request.POST['enddate']
    trip_to_update.plan = request.POST['plan']
    trip_to_update.save()

    return redirect("/dashboard/" + str(trip_id))
...
def delete_book(request, book_id):
    this_trip = Trip.objects.get(id=book_id)
    this_trip.delete()
